...
export class ItemsService {
  ...
  // URL adaptada a tu backend de items
  ...
  crear(item: any): Observable<any> {
    const url = `${this.url}`;
    return new Observable(observer => {
      axios.post(url, item, { withCredentials: true, headers: this.headers })
      .then(response => {
  ...
  actualizar(id: string, item: any): Observable<any> {
    const url = `${this.url}/${id}`;
    return new Observable(observer => {
      axios.put(url, item, { withCredentials: true, headers: this.headers })
      .then(response => {
        observer.next(response);
        observer.complete();
      })
